...
public interface I18N {
    ...
    AtomicReference<String> locale = new AtomicReference<>(Environment.string("vat.locate")
                                                                      .map(x -> x.replace("-", "#"))
                                                                      .map(Locale::of)
    ...
    static private @Nullable JsonObject load(String prefix) {
        return messages.computeIfAbsent(prefix, p -> {
            var name = p + "_" + locale;
            try {
                var res = I18N.class.getClassLoader().getResources(name);
                return StreamSupport.stream(
                                            Spliterators.spliteratorUnknownSize(res.asIterator(),
