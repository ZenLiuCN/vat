...
public interface ITimes {
    ...
    static int date(@Nullable LocalDate d, @Nullable ZoneId zone) {
        d = d == null ? LocalDate.now(zone == null ? ZONE.get() : zone) : d;
        return x2d(d.getYear(), d.getMonth().getValue(), d.getDayOfMonth());
    }

    static int time(@Nullable LocalTime d, @Nullable ZoneId zone) {
        d = d == null ? LocalTime.now(zone == null ? ZONE.get() : zone) : d;
        return x2d(d.getHour(), d.getMinute(), d.getSecond());
    }
    ...
    record ITime(int value) implements ITimes, Data.Comparable<ITime> ,Data.Binary{
        public ITime(JsonObject v) {
            this(Objects.requireNonNull(v.getInteger("value")));
        }
        ...
        public Buf toBuf(Buf buf) {
            return buf.i32(value);
        }
    }

    record IDate(int value) implements ITimes, Data.Comparable<IDate>,Data.Binary {
        public IDate(JsonObject v) {
            this(Objects.requireNonNull(v.getInteger("value")));
        }
        public IDate(Buf v) {
            this(v.i32());
    ...
    record IDatetime(long value) implements ITimes, Data.Comparable<IDatetime> ,Data.Binary {
        public IDatetime(JsonObject v) {
            this(Objects.requireNonNull(v.getLong("value")));
        }
        public IDatetime(Buf v) {
            this(v.i64());
        }
        public IDatetime(JsonObject v, Void ignore) {
            this(Optional.ofNullable(v.getString("value")).filter(Predicate.not(String::isBlank)).map(Long::parseLong).orElseThrow());
        }
        ...
        public IDatetime(LocalDateTime d) {
            this(datetime(d));
        }
        ...
        public Instant toInstant() {
            return toTime().toInstant(offset());
        }
    ...
    record Ticker(long value) implements Tickers, ITimes {
    ...
    record Tick(long value) implements Ticks, ITimes {
