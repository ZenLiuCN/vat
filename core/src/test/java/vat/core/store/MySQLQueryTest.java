...
@EnabledIfSystemProperty(
        named = "MYSQL_CONN",
        matches = ".+",  // 匹配任何非空值
        disabledReason = "MYSQL_CONN system property is not set or empty"
)
@Slf4j
public class MySQLQueryTest {
    ...
    public interface Certificate extends Record.Base, History {
        ...
        @Identity.Reference(value = PgQueryTest.User.class)
        long user();
    ...
    public static final class CertificateStore extends Model.Base<Long, Certificate, CertificateStore>  {
        ...
        public static final Storage<Long, Certificate, CertificateStore> STORAGE = (q, d, s) -> (CertificateStore) MODEL.copy(s)._with(q, d);
    ...
    static void setup(Vertx vertx, VertxTestContext vtc) {
        sql = MySQLBuilder.pool()
                .using(vertx)
                .connectingTo(System.getProperty("MYSQL_CONN"))
                .build();
        dialect = new MySQLDialect();
        store = UserStore.STORAGE.apply(sql, dialect, null);
        certificateStore = CertificateStore.STORAGE.apply(sql, dialect, null);
        SqlTemplate.forUpdate(sql, "TRUNCATE %s".formatted(store._name))
                .execute(Map.of())
                .onComplete(vtc.succeedingThenComplete());
    }
}
